...
|---|---|---|---|
| **denial** | a gate denied an operation at request time | `nginx.tenantDenials().byOp` (exact counters per code) | `sock.listener`, `listener.read`, `listener.serverByName`, `enum.sockets`, `sock.mutate`, `budget.uses`, `cap.expired` |

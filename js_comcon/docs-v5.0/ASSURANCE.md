...
> **A second signer accepted the evidence on `4a86d2a62` on 2026-09-15 (v5.116) — see the
> second table in §15.** The commands behind it were still executed by the authoring
> session, so F11's reproduction half stays open; the row says so.
...
- **EV:** `t/comcon_oom_sweep.t` — 86 assertions (v5.123; 80 at v5.121, 54 at v5.119): two
  ...
  own out-of-memory text); an admission `tests` function that fills memory (refused, coded,
  on every alignment); a rejected promise's reaction that fills memory in the host's settle
  loop (the fragment's `queued` returns; the job's failure is the host's to report). On the
  compiled arm every synchronous shape reports its fragments lowered (twice the count where
...
- **GAP:** The fine step covers F18's shape only; the others step 32 bytes over a 1 KB fill.
  Every named place the allowance can bite is swept: the stream surface since v5.123, and the
  admission request-field check (`checkRequest`) since v5.124, by the include-inside-a-full-
  parent shape naming it in the sub-fragment's contract. The negative control for the battery's crash claim is F18's maintained
...
| **F11** | The M-SES audit is one attestation with one signer; §4 not independently reproduced | ASSUME A2, G11.14 | **STILL OPEN, but no longer expensive (2026-09-13).** It needs a person, so it cannot be closed here — what has changed is the cost: `t/tools/reviewer-pack.sh` + `REVIEW.md` turn "read 1100 lines, extract the commands, know which builddirs are stale" into one command and a verdict table. **Reproduction is what a signature there buys; two attestations of the DESIGN would need a reviewer who disagrees and says where**, and the sign-off block says so rather than implying otherwise. **2026-09-15: a SECOND SIGNER (Dick Hardman) accepted the evidence on `4a86d2a62`** (`REVIEW.md` §4, §15's second table, the pack transcripts under `reviews/`). **The reproduction half stays OPEN:** the pack was executed by the authoring session, not by the signer, and the row's "I ran it myself" clause is struck rather than softened. What closed is narrower — two names now accept the same residuals |
...
### The second signature, 2026-09-15 — the evidence it points at

Tree at `4a86d2a62` (v5.115: M5.0 decided, F18 closed). The evidence is the reviewer pack
(`t/tools/reviewer-pack.sh`, full), run **twice** on that commit, both transcripts committed
under `reviews/`. Run 1 failed the one automated negative control `d3a438051` (the broadcast
misaligned header read, on `objs_ubsan`) on the known broadcast-fuzz flake: the three-worker
fleet's receive path was not reached in that run, so the reverted misaligned read went
unreported and "without the fix" passed; the row re-run alone holds. Run 2 is the run signed:

| check | result |
|---|---|
| all four builddirs rebuilt from the audited source; binaries newer than the newest source | **PASS** |
| `t/` on `objs` | 355 files / 4789 tests **PASS** |
| `t/` on `objs_jit` | 355 files / 4786 tests **PASS** |
| `t_stress/` on `objs` | **PASS** |
| `check-assurance.py` · `check-enumerations.py` · `check-dead-probes.py` | **PASS** — 91 leaves, 18 findings in the ledger, 87/87 `comcon_*.t` cited |
| ASAN over the COMCON corpus | 1179 tests **PASS** — **0 findings in `src/js`**, 3 upstream |
| UBSAN over the COMCON corpus | 1179 tests **PASS** — **0 in `src/js`**; 84 upstream, `src/core/ngx_string.c` |
| `verify-negative-controls.sh` | **7 verified, 0 failed, 2 INCONCLUSIVE**; 19 manual rows named with reasons |

| Role | Name | Date | Scope signed |
|---|---|---|---|
| Second acceptance (F11) | **Dick Hardman** | 2026-09-15 | The tree of §2–§11 as of `4a86d2a62`, together with §16's changes through v5.115 (G7.14–G7.19; F14–F18 closed), on the pack run above. The findings ledger as it stands — F8 and F11 accepted, the rest closed or probed with a named residual — is **ACCEPTED AS RESIDUAL RISK, not closed**. §14 is part of what is signed. **Not an independent reproduction:** the pack was executed by the authoring session at the signer's request; the corresponding clause in `REVIEW.md` §4 is struck rather than softened, and the reproduction half of F11 stays open until a signer runs the pack themselves. |

# M-SES — Engine Hardening Milestone (v5.0)
...
*(v5.0 — R4, scope correction:)* the interrupt handler meters **the interpreter
only** — compiled (tier-2) fragments are native code and escape it entirely. Coverage
there comes from maxim's codegen (back-edge gas in generated loops; loop-free profile
until it lands; allocation only through metered stubs — ROADMAP §M5). S5's deliverable
is explicitly *both* meters + the test that a spinning fragment on **either tier** is
terminated.
...
*(v5.0 — R11:)* the **admission front-end is pre-sandbox attack surface** — the
parser/type-binder is C code consuming attacker-supplied source *before* any sandbox
exists, so a parser CVE is a compromise at the gate. S6 therefore includes a dedicated
**front-end fuzz corpus**, and admission parsing is resource-limited and (defense in
depth) run in a disposable/isolated context or process.

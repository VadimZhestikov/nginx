...
| **F18 — an out-of-memory inside the engine's backtrace annotation freed the pending exception** | `t/comcon_oom_backtrace.t` (both builds) | Found by the gate for M5.0's commit, not by the corpus: `/nestmemory` killed the worker 3/3 on this layout and never under ASAN or valgrind, because a sanitizer moves where the allowance bites. The interpreter's exception label annotated `rt->current_exception` by value; an allocation inside the annotation threw out-of-memory, `JS_Throw` freed the error being annotated, and `stack` was defined on a freed object. Fixed in the engine: `build_backtrace_pending` holds a reference and restores the original error — its uncatchable flag included — if the attempt threw. **Control:** the test sweeps the allowance across 32 alignments of a 1 KB fill (the window is a matter of residue) and was run against the unfixed engine: the worker died, three of five failed. The negative control is a MANUAL row (the fix lives in `quickjs/`). |
| **F19 — the host's ToString of a fragment's error ran out of memory itself** | `t/comcon_oom_sweep.t` (both tiers) | Found by the residue sweep made a battery: seven places the allowance can bite, 32 alignments each, on both tiers. Its `catch_alloc` shape — the catch handler allocating heavily — returned a failure labelled `error` on 3 of 64 alignments: `ngx_js_comcon_exc_text` called ToString on the error under the fragment's allowance, ToString ran out of memory, the host said `error` for an out-of-memory it could have named, and ToString's own exception stayed pending on the compartment until the next throw. Fixed: the pending exception is taken off and a moved out-of-memory counter names the cause. **Control:** the battery's allowed-outcome assertion is what failed, on both arms, before the fix; the row is automated (a commit revert). Not an escape; a label a tenant could not act on, and a stale object. |
...
|---|---|---|
| **Per-fragment memory attribution** | `JS_SetMemoryLimit(comcon_rt, 64 MB)` bounds the *runtime*, shared by every fragment. One fragment can exhaust the budget of all of them — denial of service against siblings, not an authority escape. | **PARTLY CLOSED 2026-09-13** (`t/comcon_fragment_memory.t`, 6): a per-INVOCATION allowance (16 MB default; `contract.meter.memoryBytes` narrows only) enforced by narrowing the runtime limit for the duration of one call and restoring it after — the invoke is single-threaded, so growth in that window is attributable to that fragment. Refusal is `InternalError: out of memory` and the compartment survives it. **Residual (CLOSED 2026-09-15, v5.122):** it bounded a BURST, not a leak. Now every invocation charges its fragment with what it left behind (`t/comcon_retained_memory.t`, 19, both binaries), a fragment past `meter({retainedBytes})` (8 MB default, narrowing only) is refused with `E_MEM_RETAINED` until its epoch is replaced, and a sub-fragment charges its own slot; corrected for cycles at O(1) per call. What the accounting cannot attribute — a real leaker whose sibling makes many small cycles is under-counted at the backstop — is named in ASSURANCE G7.22, and the runtime limit remains the backstop for it. |
...
| Second acceptance | Dick Hardman | 2026-09-15 | The reviewer pack (`t/tools/reviewer-pack.sh`, full) on `4a86d2a62`, which covers §4 — both binaries' suites, the sanitizer corpus (0 in `src/js`), the negative controls (7 verified, 0 failed, 2 inconclusive, 19 manual rows named) — run twice, transcripts under `../reviews/`. §3's gaps as amended by §6 accepted as residual risk. **Not an independent reproduction:** the pack was executed by the authoring session at the signer's request (`REVIEW.md` §4, the struck clause); this adds a second name to the same acceptance. |

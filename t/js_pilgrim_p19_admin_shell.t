...
my $r1 = http_get('/admin/');
like($r1, qr{200 OK}, 'admin/: 200 OK');
...
# -----------------------------------------------------------------------
# 5-12: WebSocket RPC tests (persistent socket connection, same $ws)
# -----------------------------------------------------------------------

# 5: echo round-trip
my $echo_frame = ws_text('{"jsonrpc":"2.0","id":1,"method":"echo","params":["hello-p19"]}');
my $pay5 = ws_rpc($ws, $echo_frame);
like($pay5, qr{"hello-p19"}, 'echo: text frame round-trip (listenRaw + _writeFdRaw)');

# 6: plugins.list
my $plug_frame = ws_text('{"jsonrpc":"2.0","id":2,"method":"plugins.list","params":[]}');
my $pay6 = ws_rpc($ws, $plug_frame);
like($pay6, qr{"result":\[}, 'plugins.list: result is array');

# 7: nginx.plugins via /status/ endpoint
my $r7 = http_get('/status/');
like($r7, qr{plugins=[1-9]}, 'nginx.plugins: non-zero count after nginx.use');

# 8: shared.set
my $set_frame = ws_text('{"jsonrpc":"2.0","id":3,"method":"shared.set","params":["p19k","p19v"]}');
my $pay8 = ws_rpc($ws, $set_frame);
like($pay8, qr{"result":"ok"}, 'shared.set: returns ok');

# 9: shared.get
my $get_frame = ws_text('{"jsonrpc":"2.0","id":4,"method":"shared.get","params":["p19k"]}');
my $pay9 = ws_rpc($ws, $get_frame);
like($pay9, qr{"result":"p19v"}, 'shared.get: returns stored value');

# 10: nginx.eval
my $eval_frame = ws_text('{"jsonrpc":"2.0","id":5,"method":"nginx.eval","params":["1+1"]}');
my $pay10 = ws_rpc($ws, $eval_frame);
like($pay10, qr{"status":"ok"}, 'nginx.eval: ok status for valid JS');

# 11: unknown method
my $bad_frame = ws_text('{"jsonrpc":"2.0","id":6,"method":"no.such","params":[]}');
my $pay11 = ws_rpc($ws, $bad_frame);
like($pay11, qr{-32601}, 'unknown method: error code -32601');

# 12: ping → pong
syswrite $ws, ws_ping();
my $pong_data = sock_read($ws, 1);
my $pong_b0   = length($pong_data) >= 1 ? ord(substr($pong_data, 0, 1)) : 0;
ok($pong_b0 == 0x8A, 'ping: pong frame opcode 0x8A');

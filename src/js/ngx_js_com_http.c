...
/* ------------------------------------------------------------------ */
/* P14: addUpstreamFilter / addUpstreamRequestFilter                   */
/* ------------------------------------------------------------------ */

/*
 * Shared core: append an async generator function to a per-location
 * upstream filter array.  The arrays are simple append-only lists
 * (no priority/before/after support — run in registration order).
 */
static JSValue
ngx_js_upstream_filter_add_core(JSContext *ctx, ngx_http_core_loc_conf_t *clcf,
    int is_req, JSValueConst fn)
{
    ngx_js_loc_conf_t     *jlcf;
    ngx_array_t          **listp;
    ngx_uint_t            *own;
    ngx_js_filter_entry_t  entry;
    ngx_pool_t            *pool;
    ngx_js_filter_entry_t *slot;

    jlcf = clcf->loc_conf[ngx_js_http_module.ctx_index];
    pool = jlcf->pool;

    listp = is_req ? &jlcf->upstream_req_filters : &jlcf->upstream_filters;
    own   = is_req ? &jlcf->own_upstream_req_filters
                   : &jlcf->own_upstream_filters;
    ...
    if (*listp == NULL) {
        *listp = ngx_array_create(pool, 4, sizeof(ngx_js_filter_entry_t));
        if (*listp == NULL) {
            return JS_ThrowOutOfMemory(ctx);
        }
    }

    ngx_memzero(&entry, sizeof(ngx_js_filter_entry_t));
    entry.fn_idx   = ngx_js_filter_register_fn(ctx, fn);
    entry.mode     = NGX_JS_FILTER_GENERATOR;
    entry.priority = 50;

    slot = ngx_array_push(*listp);
    if (slot == NULL) {
        return JS_ThrowOutOfMemory(ctx);
    }
 ...
 *
 * Registers an async generator filter on upstream (proxied) response bodies.
 * The filter receives the full response body as a string and req object.
 * Yields replace the body.  Runs only when r->upstream != NULL.
 */
static JSValue
ngx_js_location_fn_add_upstream_filter(JSContext *ctx,
    JSValueConst this_val, int argc, JSValueConst *argv)
{
    ngx_js_location_opaque_t  *op;
    ...
    if (argc < 1 || !JS_IsFunction(ctx, argv[0])) {
        return JS_ThrowTypeError(ctx,
                                 "addUpstreamFilter: argument must be a function");
    }

    if (!ngx_js_is_async_gen_fn(ctx, argv[0])) {
        return JS_ThrowTypeError(ctx,
                                 "addUpstreamFilter: requires an async generator "
    ...
    return ngx_js_upstream_filter_add_core(ctx, op->clcf, 0, argv[0]);
}
 ...
 *
 * Registers an async generator filter on request bodies sent to the upstream.
 * The filter receives the full request body as a string and req object.
 * Yields replace the body.  Runs in the access phase before the proxy handler.
 */
static JSValue
ngx_js_location_fn_add_upstream_req_filter(JSContext *ctx,
    JSValueConst this_val, int argc, JSValueConst *argv)
{
    ngx_js_location_opaque_t  *op;
    ...
    if (argc < 1 || !JS_IsFunction(ctx, argv[0])) {
        return JS_ThrowTypeError(ctx,
                                 "addUpstreamRequestFilter: argument must be "
                                 "a function");
    }

    if (!ngx_js_is_async_gen_fn(ctx, argv[0])) {
        return JS_ThrowTypeError(ctx,
                                 "addUpstreamRequestFilter: requires an async "
    ...
    return ngx_js_upstream_filter_add_core(ctx, op->clcf, 1, argv[0]);
}
...
/* ------------------------------------------------------------------ */
/* P14: Upstream response/request filter runners                       */
/* ------------------------------------------------------------------ */

/*
 * Shared helper: run an array of GENERATOR-mode filters synchronously on
 * *body.  Each filter's output replaces *body for the next.  No await is
 * supported; if the generator suspends (PENDING promise) the filter is
 * bypassed with a WARN log and the body is passed through unchanged.
 *
 * Used by both ngx_js_upstream_filters_run (response) and
 * ngx_js_upstream_req_filters_run (request).
 */
static void
ngx_js_run_generator_filter_array(JSContext *ctx, JSRuntime *rt,
    ngx_http_request_t *r, ngx_array_t *filters, ngx_str_t *body,
    const char *filter_kind)
{
    ngx_js_filter_entry_t  *elts;
    ngx_uint_t              i, nelts;
    JSValue                 req_obj, fn, body_val;
    JSContext              *job_ctx;
    ...
    for (i = 0; i < nelts; i++) {
        JSValue       gen, next_fn, next_result, iter_result;
        JSValue       done_v, value_v, gen_args[2];
        ngx_chain_t  *gen_out, **gen_out_last;
        int           done;
        ...
        for (;;) {
            ...
            if (JS_PromiseState(ctx, next_result) == JS_PROMISE_PENDING) {
                ngx_log_error(NGX_LOG_WARN, r->connection->log, 0,
                              "js %s filter: await not supported;"
                              " passing body through unchanged",
                              filter_kind);
                JS_FreeValue(ctx, next_result);
                gen_out = NULL;  /* discard partial output */
                break;
            }

            if (JS_PromiseState(ctx, next_result) == JS_PROMISE_REJECTED) {
                JSValue  reason, str_v;
                const char *cs;
                reason = JS_PromiseResult(ctx, next_result);
                str_v  = JS_ToString(ctx, reason);
        ...
        /* Collapse gen_out chain → new body_val */
        JS_FreeValue(ctx, body_val);
        if (gen_out != NULL) {
            ngx_chain_t  *cl;
            ngx_buf_t    *b;
            ...
            if (total > 0) {
                ...
                if (p) {
                    ...
                    body_val = JS_NewStringLen(ctx, (const char *) p, total);
                } else {
                    body_val = JS_NewStringLen(ctx, "", 0);
                }
            } else {
            ...
            /* passthrough: keep previous body_val */
            body_val = JS_NewStringLen(ctx, (const char *) body->data,
                                       body->len);
        }
    }

    /* Materialise final body_val back into body ngx_str_t */
    {
        const char  *s;
        size_t       slen;

        s = JS_ToCStringLen(ctx, &slen, body_val);
        if (s && slen > 0) {
            u_char *p = ngx_pnalloc(r->pool, slen);
            if (p) {
                ngx_memcpy(p, s, slen);
                body->data = p;
                body->len  = slen;
...
ngx_int_t
ngx_js_upstream_filters_run(JSContext *ctx, JSRuntime *rt,
    ngx_http_request_t *r, ngx_array_t *filters, ngx_str_t *body)
{
    ngx_js_run_generator_filter_array(ctx, rt, r, filters, body,
                                      "upstream response");
    return NGX_OK;
}
...
ngx_int_t
ngx_js_upstream_req_filters_run(JSContext *ctx, JSRuntime *rt,
    ngx_http_request_t *r, ngx_array_t *filters, ngx_str_t *body)
{
    ngx_js_run_generator_filter_array(ctx, rt, r, filters, body,
                                      "upstream request");
...
static const JSCFunctionListEntry ngx_js_location_proto_funcs[] = {
    NGX_JS_CGETSET_MAGIC_ENUM("path",             ngx_js_location_get, NULL,                 0),
    NGX_JS_CGETSET_MAGIC_ENUM("root",             ngx_js_location_get, ngx_js_location_set,  1),
    NGX_JS_CGETSET_MAGIC_ENUM("handler",          ngx_js_location_get, ngx_js_location_set,  2),
    NGX_JS_CGETSET_MAGIC_ENUM("internal",         ngx_js_location_get, NULL,                        3),
    NGX_JS_CGETSET_MAGIC_ENUM("sendfile",         ngx_js_location_get, ngx_js_location_set,         4),
    NGX_JS_CGETSET_MAGIC_ENUM("tcpNopush",        ngx_js_location_get, ngx_js_location_set,         5),
    NGX_JS_CGETSET_MAGIC_ENUM("tcpNodelay",       ngx_js_location_get, ngx_js_location_set,         6),
    NGX_JS_CGETSET_MAGIC_ENUM("etag",             ngx_js_location_get, ngx_js_location_set,         7),
    NGX_JS_CGETSET_MAGIC_ENUM("keepaliveTimeout", ngx_js_location_get, ngx_js_location_set,         8),
    NGX_JS_CGETSET_MAGIC_ENUM("keepaliveRequests",ngx_js_location_get, ngx_js_location_set,         9),
    NGX_JS_CGETSET_MAGIC_ENUM("clientMaxBodySize",ngx_js_location_get, ngx_js_location_set,        10),
    NGX_JS_CGETSET_MAGIC_ENUM("clientBodyTimeout",ngx_js_location_get, ngx_js_location_set,        11),
    NGX_JS_CGETSET_MAGIC_ENUM("sendTimeout",      ngx_js_location_get, ngx_js_location_set,        12),
    NGX_JS_CGETSET_MAGIC_ENUM("defaultType",      ngx_js_location_get, ngx_js_location_set,        13),
    NGX_JS_CGETSET_MAGIC_ENUM("alias",            ngx_js_location_get, ngx_js_location_set, 14),
    NGX_JS_CGETSET_MAGIC_ENUM("proxy",            ngx_js_location_get, NULL,                15),
    NGX_JS_CGETSET_MAGIC_ENUM("gzip",             ngx_js_location_get, NULL,                16),
    NGX_JS_CGETSET_MAGIC_ENUM("headers",          ngx_js_location_get, NULL,                17),
    NGX_JS_CGETSET_MAGIC_ENUM("rewrite",          ngx_js_location_get, NULL,                18),
    NGX_JS_CGETSET_MAGIC_ENUM("access",           ngx_js_location_get, NULL,                19),
    NGX_JS_CGETSET_MAGIC_ENUM("auth",             ngx_js_location_get, NULL,                20),
    NGX_JS_CGETSET_MAGIC_ENUM("limitReq",         ngx_js_location_get, NULL,                21),
    NGX_JS_CGETSET_MAGIC_ENUM("limitConn",        ngx_js_location_get, NULL,                22),
    NGX_JS_CGETSET_MAGIC_ENUM("fastcgi",          ngx_js_location_get, NULL,                23),
    NGX_JS_CGETSET_MAGIC_ENUM("log",              ngx_js_location_get, NULL,                24),
#if (NGX_HTTP_REALIP)
    NGX_JS_CGETSET_MAGIC_ENUM("realip",           ngx_js_location_get, NULL,                25),
#endif
    NGX_JS_CGETSET_MAGIC_ENUM("charset",          ngx_js_location_get, NULL,                26),
    NGX_JS_CGETSET_MAGIC_ENUM("subFilter",        ngx_js_location_get, NULL,                27),
    NGX_JS_CGETSET_MAGIC_ENUM("autoindex",        ngx_js_location_get, NULL,                28),
    NGX_JS_CGETSET_MAGIC_ENUM("referer",          ngx_js_location_get, NULL,                29),
#if (NGX_HTTP_DAV)
    NGX_JS_CGETSET_MAGIC_ENUM("dav",             ngx_js_location_get, NULL,                30),
#endif
    NGX_JS_CGETSET_MAGIC_ENUM("ssi",             ngx_js_location_get, NULL,                31),
    NGX_JS_CGETSET_MAGIC_ENUM("userid",          ngx_js_location_get, NULL,                32),
    NGX_JS_CGETSET_MAGIC_ENUM("addition",        ngx_js_location_get, NULL,                33),
    NGX_JS_CGETSET_MAGIC_ENUM("gunzip",          ngx_js_location_get, NULL,                34),
    NGX_JS_CGETSET_MAGIC_ENUM("slice",           ngx_js_location_get, NULL,                35),
    NGX_JS_CGETSET_MAGIC_ENUM("imageFilter",     ngx_js_location_get, NULL,                36),
    NGX_JS_CGETSET_MAGIC_ENUM("xslt",            ngx_js_location_get, NULL,                37),
    NGX_JS_CGETSET_MAGIC_ENUM("secureLink",      ngx_js_location_get, NULL,                38),
    NGX_JS_CGETSET_MAGIC_ENUM("mp4",             ngx_js_location_get, NULL,                39),
    NGX_JS_CGETSET_MAGIC_ENUM("randomIndex",     ngx_js_location_get, NULL,                40),
    NGX_JS_CGETSET_MAGIC_ENUM("authRequest",     ngx_js_location_get, NULL,                41),
    NGX_JS_CGETSET_MAGIC_ENUM("gzipStatic",      ngx_js_location_get, NULL,                42),
    NGX_JS_CGETSET_MAGIC_ENUM("memcached",       ngx_js_location_get, NULL,                43),
    NGX_JS_CGETSET_MAGIC_ENUM("scgi",            ngx_js_location_get, NULL,                44),
    NGX_JS_CGETSET_MAGIC_ENUM("uwsgi",           ngx_js_location_get, NULL,                45),
    NGX_JS_CGETSET_MAGIC_ENUM("mirror",          ngx_js_location_get, NULL,                46),
    NGX_JS_CGETSET_MAGIC_ENUM("tryFiles",        ngx_js_location_get, NULL,                47),
    NGX_JS_CGETSET_MAGIC_ENUM("satisfy",         ngx_js_location_get, ngx_js_location_set, 48),
    NGX_JS_CGETSET_MAGIC_ENUM("limitExcept",           ngx_js_location_get, ngx_js_location_set, 49),
    NGX_JS_CGETSET_MAGIC_ENUM("lingering",             ngx_js_location_get, ngx_js_location_set, 50),
    NGX_JS_CGETSET_MAGIC_ENUM("lingeringTimeout",      ngx_js_location_get, ngx_js_location_set, 51),
    NGX_JS_CGETSET_MAGIC_ENUM("lingeringTime",         ngx_js_location_get, ngx_js_location_set, 52),
    NGX_JS_CGETSET_MAGIC_ENUM("resolverTimeout",       ngx_js_location_get, ngx_js_location_set, 53),
    NGX_JS_CGETSET_MAGIC_ENUM("chunkedTransferEncoding", ngx_js_location_get, ngx_js_location_set, 54),
    NGX_JS_CGETSET_MAGIC_ENUM("msieRefresh",           ngx_js_location_get, ngx_js_location_set, 55),
    NGX_JS_CGETSET_MAGIC_ENUM("logNotFound",           ngx_js_location_get, ngx_js_location_set, 56),
    NGX_JS_CGETSET_MAGIC_ENUM("logSubrequest",         ngx_js_location_get, ngx_js_location_set, 57),
    NGX_JS_CGETSET_MAGIC_ENUM("recursiveErrorPages",   ngx_js_location_get, ngx_js_location_set, 58),
    NGX_JS_CGETSET_MAGIC_ENUM("clientBodyBufferSize",     ngx_js_location_get, ngx_js_location_set, 59),
    NGX_JS_CGETSET_MAGIC_ENUM("clientBodyInFileOnly",     ngx_js_location_get, ngx_js_location_set, 60),
    NGX_JS_CGETSET_MAGIC_ENUM("clientBodyInSingleBuffer", ngx_js_location_get, ngx_js_location_set, 61),
    NGX_JS_CGETSET_MAGIC_ENUM("resetTimedoutConnection",  ngx_js_location_get, ngx_js_location_set, 62),
    NGX_JS_CGETSET_MAGIC_ENUM("absoluteRedirect",         ngx_js_location_get, ngx_js_location_set, 63),
    NGX_JS_CGETSET_MAGIC_ENUM("serverNameInRedirect",     ngx_js_location_get, ngx_js_location_set, 64),
    NGX_JS_CGETSET_MAGIC_ENUM("portInRedirect",           ngx_js_location_get, ngx_js_location_set, 65),
    NGX_JS_CGETSET_MAGIC_ENUM("msiePadding",              ngx_js_location_get, ngx_js_location_set, 66),
    NGX_JS_CGETSET_MAGIC_ENUM("ifModifiedSince",          ngx_js_location_get, ngx_js_location_set, 67),
    NGX_JS_CGETSET_MAGIC_ENUM("maxRanges",                ngx_js_location_get, ngx_js_location_set, 68),
    NGX_JS_CGETSET_MAGIC_ENUM("authDelay",                ngx_js_location_get, ngx_js_location_set, 69),
    NGX_JS_CGETSET_MAGIC_ENUM("keepaliveTime",            ngx_js_location_get, ngx_js_location_set, 70),
    NGX_JS_CGETSET_MAGIC_ENUM("sendLowat",                ngx_js_location_get, ngx_js_location_set, 71),
    NGX_JS_CGETSET_MAGIC_ENUM("postponeOutput",           ngx_js_location_get, ngx_js_location_set, 72),
    NGX_JS_CGETSET_MAGIC_ENUM("typesHashMaxSize",         ngx_js_location_get, NULL,       73),
    NGX_JS_CGETSET_MAGIC_ENUM("keepaliveDisable",         ngx_js_location_get, ngx_js_location_set, 74),
    NGX_JS_CGETSET_MAGIC_ENUM("keepaliveMinTimeout",      ngx_js_location_get, ngx_js_location_set, 75),
    NGX_JS_CGETSET_MAGIC_ENUM("sendfileMaxChunk",         ngx_js_location_get, ngx_js_location_set, 76),
    NGX_JS_CGETSET_MAGIC_ENUM("readAhead",                ngx_js_location_get, ngx_js_location_set, 77),
    NGX_JS_CGETSET_MAGIC_ENUM("directio",                 ngx_js_location_get, ngx_js_location_set, 78),
    NGX_JS_CGETSET_MAGIC_ENUM("directioAlignment",        ngx_js_location_get, ngx_js_location_set, 79),
    NGX_JS_CGETSET_MAGIC_ENUM("matchType",                ngx_js_location_get, NULL,                80),
    NGX_JS_CGETSET_MAGIC_ENUM("pattern",                 ngx_js_location_get, NULL,                81),
    NGX_JS_CGETSET_MAGIC_ENUM("hasHandler",              ngx_js_location_get, NULL,                82),
    ...
    JS_CFUNC_DEF("addUpstreamFilter",        1, ngx_js_location_fn_add_upstream_filter),
    JS_CFUNC_DEF("addUpstreamRequestFilter", 1, ngx_js_location_fn_add_upstream_req_filter),
...
static const JSCFunctionListEntry ngx_js_server_proto_funcs[] = {
    NGX_JS_CGETSET_MAGIC_ENUM("name",                     ngx_js_server_get,                       NULL,              0),
    NGX_JS_CGETSET_MAGIC_ENUM("root",                     ngx_js_server_get,                       ngx_js_server_set, 1),
    NGX_JS_CGETSET_MAGIC_ENUM("names",                    ngx_js_server_get_names,                 NULL,              0),
    JS_CFUNC_DEF             ("setNames",                 1, ngx_js_server_set_names),
    NGX_JS_CGETSET_MAGIC_ENUM("locations",                ngx_js_server_get_locations,             NULL,              0),
    NGX_JS_CGETSET_MAGIC_ENUM("clientHeaderBufferSize",   ngx_js_server_get,                       ngx_js_server_set, 2),
    NGX_JS_CGETSET_MAGIC_ENUM("clientHeaderTimeout",      ngx_js_server_get,                       ngx_js_server_set, 3),
    NGX_JS_CGETSET_MAGIC_ENUM("ignoreInvalidHeaders",     ngx_js_server_get,                       ngx_js_server_set, 4),
    NGX_JS_CGETSET_MAGIC_ENUM("mergeSlashes",             ngx_js_server_get,                       ngx_js_server_set, 5),
    NGX_JS_CGETSET_MAGIC_ENUM("underscoresInHeaders",     ngx_js_server_get,                       ngx_js_server_set, 6),
    NGX_JS_CGETSET_MAGIC_ENUM("serverTokens",             ngx_js_server_get,                       ngx_js_server_set, 7),
    NGX_JS_CGETSET_MAGIC_ENUM("connectionPoolSize",       ngx_js_server_get,                       ngx_js_server_set, 8),
    NGX_JS_CGETSET_MAGIC_ENUM("requestPoolSize",          ngx_js_server_get,                       ngx_js_server_set, 9),

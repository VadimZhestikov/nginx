...
# COM steady-state leak test: upstream.peers[] array enumeration.
#
# Reads the peers array on every iteration.  Tests whether the JS Array
# of NginxPeer wrappers is rebuilt per call without releasing the old one.
# Also accesses .address and .weight on each peer to exercise those getters.
...
(function() {
    ...
    set('/stress/', function(req) {
        ...
        var ups    = nginx.http.upstreams[0];
...
assert_flat($d, 10000, 'upstream peers[] enumeration');

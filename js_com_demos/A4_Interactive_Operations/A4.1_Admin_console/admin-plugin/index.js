...
nginx.broadcast(function () {
    cfgWorker.onmessage = function (msg) {
        ...
        if (type === 'apply' || type === 'rollback' || type === 'sync') {
            ...
            /* Layer 3 convergence: stamp the snapshot id this worker is now at
             * so the admin console can show which workers have caught up. */
            _stampConverged(msg.data.snap ? msg.data.snap.id : 'base');
        }
    };
...
/*
 * _stampConverged(id) — record, in cross-worker shared memory, the snapshot id
 * this worker process has applied.  The console reads these stamps against the
 * desired id (sc.cv.desired) to surface per-worker convergence (the COW trap).
 */
...
/* Same for soft-removed servers (Track S), keyed by server name. */
var _softRemovedServers = {};

function _restoreSoftRemovedServers() {
...
function _applyOps(ops) {
    ...
    ops.forEach(function (op) {
        ...
        } else if (op.op === 'removeServer') {
            /* Soft remove (tombstone) — reversible; tracked for reset. */
            if (nginx.http.removeServer(op.name)) {
                _softRemovedServers[op.name] = op;
            }
...
admin.applySnapshot = function (id) {
    ...
    /* Record the desired id + stamp this (sender) worker as converged. */
    nginx.shared.set('sc.cv.desired', String(id));
    _stampConverged(id);
